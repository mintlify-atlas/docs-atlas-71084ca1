...
description: "MELIKA medications catalog: OTC and Rx drugs linked to medical specialties, filterable by type, specialty ID, or name for use in clinical prescriptions."
---
...
<Tabs>
  ...
  <Tab title="Filter by Specialty">
    Return medications linked to a specific specialty using its **numeric ID**. Pass `id_especialidad` as an integer matching `especialidades.id`. The string value `"Todos"` is treated as no filter.
  ...
  <Tab title="Search by Name">
    ...
    This matches medications where either `nombre_comercial` or `principio_activo` contains `"metform"`, such as `"Metformina"`.
  </Tab>
  <Tab title="Combined Filters">
    ...
    Returns only active Rx medications under specialty `1` (e.g., Cardiología) that match `"atorva"` in name or ingredient.
  </Tab>
</Tabs>
...
The endpoint queries `especialidades.nombre` dynamically via a `JOIN` on `medicamentos.id_especialidad`. Only specialties with at least one medication where `activo = TRUE` appear.
