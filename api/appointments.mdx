---
title: "Appointments API — Booking, Calendar & Cancellation"
sidebarTitle: "Appointments"
description: "REST endpoints for the MELIKA appointment lifecycle: create, list, calendar view, cancel, and delete bookings with concurrency-safe slot protection."
---
...
<ResponseField name="[]" type="array of objects">
  <Expandable title="Appointment object fields">
    ...
    <ResponseField name="fecha" type="string">Date in `YYYY-MM-DD` format (formatted via SQL `TO_CHAR` to avoid timezone drift).</ResponseField>
    <ResponseField name="hora_inicio" type="string">Start time (`HH:MM`).</ResponseField>
    <ResponseField name="hora_fin" type="string">End time resolved from the linked `franja_horaria` record (`HH:MM`).</ResponseField>
    ...
    <ResponseField name="medico_apellido" type="string">Doctor's first surname.</ResponseField>
...
<ResponseField name="[]" type="array of objects">
  <Expandable title="Event object fields">
    <ResponseField name="id" type="string">Appointment identifier cast to string (e.g. `"201"`) — FullCalendar expects a string event ID.</ResponseField>
    <ResponseField name="title" type="string">Display label shown on the calendar tile, formatted as `"{Estado} · {Especialidad}"` (e.g. `"Pendiente · Medicina Interna"`).</ResponseField>
    <ResponseField name="start" type="string">
      Event start datetime assembled by concatenating `fecha_str` and `hora_inicio` with a `T` separator: `"YYYY-MM-DDTHH:MM"`. No seconds component — `hora_inicio` is stored as `HH:MM`.
    </ResponseField>
    <ResponseField name="backgroundColor" type="string">
      Hex fill color driven by appointment state:
      - `pendiente` → `#B45309` (amber)
      - `completada` → `#1A7A52` (green)
      - `no_asistio` → `#6B7280` (gray)
    </ResponseField>
    <ResponseField name="borderColor" type="string">
      Distinct darker border color per state:
      - `pendiente` → `#92400E`
      - `completada` → `#145C3E`
      - `no_asistio` → `#4B5563`
    </ResponseField>
    ...
    <ResponseField name="extendedProps" type="object">
      ...
      <Expandable title="extendedProps fields">
        ...
        <ResponseField name="tipo_consulta" type="string">`presencial` or `teleconsulta`.</ResponseField>
        <ResponseField name="medico_nombre" type="string">Doctor's full name (`nombre + primer_apellido`).</ResponseField>
        <ResponseField name="especialidad" type="string">Specialty name.</ResponseField>
        <ResponseField name="motivo" type="string">Visit reason, if provided.</ResponseField>
        <ResponseField name="tarifa" type="number">Consultation fee charged.</ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>
...
[
  {
    "id": "201",
    "title": "Pendiente · Medicina Interna",
...
<ResponseField name="cita" type="object">
  ...
  <Expandable title="cita fields">
    ...
    <ResponseField name="razon_cancelacion" type="string">The trimmed reason provided in the request.</ResponseField>
