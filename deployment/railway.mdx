  ...
  <Step title="Deploy the backend service">
    ...
    <Tabs>
      ...
      <Tab title="Environment variables">
        Add these variables in **Settings → Variables** for the `server` service. Use Railway variable references (e.g. `${{Postgres.PGHOST}}`) to pull values from the Postgres service without copying secrets manually:
        ...
        Railway injects `PORT` automatically — you do not need to set it.
      </Tab>
    </Tabs>
    ...
    <Tabs>
      <Tab title="Railway shell (recommended)">
        ...
        psql "postgresql://$DB_USER:$DB_PASSWORD@$DB_HOST:$DB_PORT/$DB_NAME" \
          -f src/database/melika_db.sql
        ```

        Railway's internal network gives the shell direct access to Postgres via the injected `DB_*` variables.
      </Tab>
      <Tab title="Local psql with Railway URL">
        Copy the connection details from the Postgres service's **Connect** tab (use the public URL that includes the Railway proxy port). Then run locally:
