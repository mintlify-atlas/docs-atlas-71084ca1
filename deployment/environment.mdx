...
| `DATABASE_URL` | Full PostgreSQL connection string. Railway injects this automatically when you link a Postgres service. | `postgresql://user:pass@host:5432/melika_db` | **Yes** |
...
| `GMAIL_USER` | Gmail address that sends verification and notification emails. | `noreply@yourapp.com` | **Yes** |
...
MELIKA uses Gmail's OAuth2 flow instead of a plain SMTP password so that Google cannot block transactional email as "less secure app access". Follow these steps once to obtain the four Gmail-related variables.
...
PORT=3000
DATABASE_URL=postgresql://postgres:password@localhost:5432/melika_db
...
- Store `DATABASE_URL` and `JWT_SECRET` only in secure secret stores — never in source code or public repositories.
